...
2. The page sends that to the server's `/api/parse` endpoint, which uses the **Google Gemini API** to turn it into structured exercises (sets, reps, weight) **plus editable coaching cues** for each movement. The coach can also fill in an optional **Athlete context** anchor (injuries, rehab status, level, goals, equipment — or paste the athlete's exported history) and the cues are **tailored to that athlete**. This step is fast, so the review screen appears right away.
